...
"""
Layer:  Linear layer for float16 input and int8 output
Author: cxz21
Data:   2025/07/13
"""
class LinearFunc(torch.autograd.Function):
    ...
    def forward(
        ...
        ppu_output = quant_output * scaling_factor

        output = convert_int8(ppu_output).to(input.device)

        ctx.save_for_backward(
            quant_input, quant_weight, input_delta, parameter, weight_delta, next_delta
        )
        return output.to(torch.float)

    @staticmethod
    def backward(ctx, grad_output: torch.Tensor):
        quant_input, quant_weight, input_delta, parameter, weight_delta, next_delta = ctx.saved_tensors

        dequant_input = quant_input * input_delta
        dequant_weight = quant_weight * (2.0 ** (-weight_delta + 1)) * parameter
        dequant_grad_output = grad_output / next_delta
...
class Linear(torch.nn.Module):
    def __init__(self, in_feature: int, out_feature: int, bias: bool = False):
        ...
        self.output_delta = torch.nn.Parameter(torch.full([], 0.0), requires_grad=False)
    ...
    def forward(self, input: torch.Tensor, parameter: torch.Tensor, dequant_input: torch.Tensor):
        if self.training is True:
            ...
            input_ = dequant_input.clone().detach()
            weight_ = self.weight.clone().detach()
            output_ = input_ @ weight_.transpose(0, 1)
            self.output_delta.data = toint8.int8_init_scale(output_) #* 2

            self.scaling_factor =  (self.input_delta * (2.0 ** (-self.weight_delta + 1)) / self.output_delta  * 2**13)
            ...
            input_ = dequant_input.clone().detach().to(torch.float)
            weight_ = self.weight.clone().detach()
            output_ = input_ @ weight_.transpose(0, 1)
            self.output_delta.data = toint8.int8_init_scale(output_) #* 2
        ...
        # quant_output = quant_output.to(torch.float16)
        # ppu_output = quant_output * self.scaling_factor

        # output = (self.convert_int8.convert(ppu_output).to("cuda") - ppu_output).detach() + ppu_output

        return output.to(torch.float), self.output_delta
...
if __name__ == "__main__":
    ...
    custom_output,  next_delta = custom_linear(x1, 16, x1.detach().clone()*16)
    # custom_output,  next_delta = custom_linear(x1, 1, x1)
    x3 = x2*16
    torch_output = torch_linear(x3) / next_delta.clone().detach()
